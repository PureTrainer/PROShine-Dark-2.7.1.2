...
namespace PROProtocol
{
    public class Map
    {
        ...
        public Map(byte[] content, string mapName)
        {
            using (MemoryStream stream = new MemoryStream(content))
            {
                using (BinaryReader reader = new BinaryReader(stream))
                {
                    ...
                    for (int i = 0; i < npcCount; ++i)
                    {
                        ...
                        {
                            if (mapName.Contains("Goldenrod Underground"))
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else if (mapName.Contains("Route 124"))
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else if (mapName == "Lilycove City")
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else if(mapName.Contains("Underground Warehouse"))
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else if(mapName.Contains("Player"))
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else if (mapName.Contains("Route 119A"))
                            {
                                OriginalNpcs.Add(new Npc(npcId, npcName, isBattler, type, x, y, viewDirection, losLength, path));
                            }
                            else
